...
int total;
double tip, tax;
double midtip, midtax;
double per;

per = 100;

midtip=(tip_percent/per);
midtax=(tax_percent/per);

tip = midtip*meal_cost;
tax = midtax*meal_cost;

total = round( meal_cost  + tip + tax); 
/* total is defined as int variable, so any floating point values are rounded off*/
printf("%d", total);
}
